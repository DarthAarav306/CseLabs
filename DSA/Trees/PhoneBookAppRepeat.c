...
// Function to load student records from a file and insert into BST
Student* loadFromFile(const char* filename) {
    ...
    while (fscanf(file, "%d %s %s %f %s", &roll_no, name, section, &cgpa, phone) == 5) {
...
int main() {
    ...
    do {
        printf("\nMenu:\n");
        printf("1. Load Student Records from File\n");
